...
{
    /* SHA1class initialization constants */
    digest[0] = 0x67452301;
    digest[1] = 0xefcdab89;
    digest[2] = 0x98badcfe;
    digest[3] = 0x10325476;
...
/*
 * (R0+R1), R2, R3, R4 are the different operations used in SHA1class
 */
...
void SHA1class::update(const std::string &s)
{
    std::istringstream is(s);
    update(is);
}

void SHA1class::update(const char buff[], int bufflen){
	std::istringstream is(std::string(buff, bufflen));
	update(is);
}

void SHA1class::update(std::istream &is)
{
    while (true)
    {
...
std::string SHA1class::from_file(const std::string &filename)
{
